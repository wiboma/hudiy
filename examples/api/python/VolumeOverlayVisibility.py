...
class EventHandler(ClientEventHandler):
    ...
    def toggle_overlay_visibility(self, client):
        ...
        set_volume_overlay_visibility = hudiy_api.SetVolumeOverlayVisibility()
        set_volume_overlay_visibility.visibility = hudiy_api.OVERLAY_VISIBILITY_ALWAYS if self._visible else hudiy_api.OVERLAY_VISIBILITY_NONE
        client.send(hudiy_api.MESSAGE_SET_VOLUME_OVERLAY_VISIBILITY, 0,
                    set_volume_overlay_visibility.SerializeToString())
...
def main():
    client = Client("volume overlay visibility example")
